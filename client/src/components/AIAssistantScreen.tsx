...
export default function AIAssistantScreen({ 
  ...
  return (
    <div className="h-screen flex flex-col text-white relative overflow-hidden">
      {/* Efectos de fondo */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#36219c] to-[#36219c]/80 opacity-90 pointer-events-none z-0"></div>
      
      {/* Elementos decorativos de fondo */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        {/* Círculos de color */}
        <div className="absolute top-[10%] right-[10%] w-40 h-40 rounded-full bg-[#FF66C4]/20 blur-xl"></div>
        <div className="absolute bottom-[20%] left-[15%] w-56 h-56 rounded-full bg-[#16deca]/15 blur-xl"></div>
        <div className="absolute top-[40%] left-[5%] w-32 h-32 rounded-full bg-[#FF9F5B]/20 blur-xl"></div>
        ...
        {[...Array(6)].map((_, i) => (
          <div 
            ...
            style={{
              ...
              animation: `floatParticle ${Math.random() * 10 + 20}s linear infinite, fadeInOut ${Math.random() * 5 + 5}s ease-in-out infinite`
            }}
          />
        ))}
      </div>
      ...
      <div className="absolute inset-0 bg-shimmer opacity-20 pointer-events-none z-0"></div>
      
      {/* Header */}
      <header className="bg-[#36219c]/70 backdrop-blur-md border-b border-[#16deca]/30 py-4 px-5 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center">
          <button 
            onClick={onBack} 
            className="mr-3 hover:bg-white/10 p-2 rounded-full transition-all duration-300 hover:scale-105 border border-transparent hover:border-[#16deca]/30"
          >
            <RemixIcon name="arrow-left-line" size="xl" />
          </button>
          <div className="flex flex-col">
            <h1 className="font-heading text-xl font-bold bg-gradient-to-r from-[#16deca] to-[#FF66C4] bg-clip-text text-transparent">
              AromaSfera AI
            </h1>
            <span className="text-xs text-[#f0f0f0]/70 font-accent">Tu asistente de fragancias personal</span>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          <button className="w-9 h-9 flex items-center justify-center rounded-full bg-[#36219c]/60 hover:bg-[#36219c]/80 transition-colors border border-[#16deca]/30 hover:border-[#16deca]/60 hover:scale-105 duration-300">
            <RemixIcon name="question-line" />
          </button>
          <div className="w-9 h-9 overflow-hidden rounded-full bg-gradient-to-r from-[#FF66C4] to-[#16deca] flex items-center justify-center border border-[#16deca]/30 shadow-lg shadow-[#FF66C4]/30 animate-float">
      ...
      <div ref={chatAreaRef} className="flex-1 overflow-y-auto p-4 bg-transparent">
          ...
          <div key={index} className="mb-5">
            {message.sender === "assistant" ? (
              <div className="flex items-start animate-fadeIn">
                <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#16deca] to-[#36219c] flex-shrink-0 flex items-center justify-center shadow-lg shadow-[#16deca]/20 border border-[#16deca]/30">
                  <RemixIcon name="robot-line" className="text-white" />
                </div>
                <div className="ml-3 bg-[#36219c]/40 backdrop-blur-md p-4 rounded-2xl rounded-tl-none max-w-[80%] shadow-xl border border-[#16deca]/30 group hover:border-[#16deca]/50 transition-all">
                  <p className="text-sm leading-relaxed">{message.text}</p>
                  <span className="text-xs text-[#f0f0f0]/50 opacity-0 group-hover:opacity-100 transition-opacity">AromaSfera AI • {new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                </div>
              </div>
            ) : (
              <div className="flex justify-end items-start animate-fadeInRight">
                <div className="mr-3 bg-gradient-to-r from-[#FF66C4]/20 to-[#FF9F5B]/20 backdrop-blur-md p-4 rounded-2xl rounded-tr-none max-w-[75%] border border-[#FF66C4]/30 shadow-lg group hover:border-[#FF66C4]/50 transition-all">
                  <p className="text-sm leading-relaxed">{message.text}</p>
                  <span className="text-xs text-[#f0f0f0]/50 text-right block opacity-0 group-hover:opacity-100 transition-opacity">Tú • {new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                </div>
                <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#FF66C4] to-[#FF9F5B] flex-shrink-0 overflow-hidden shadow-lg shadow-[#FF66C4]/20 border border-[#FF66C4]/30 flex items-center justify-center">
        ...
        {isListening && (
          <div className="flex mb-4 items-center animate-pulse">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-r from-[#FF66C4] to-[#FF9F5B] flex-shrink-0 flex items-center justify-center shadow-lg shadow-[#FF66C4]/30 border border-[#FF66C4]/40">
              <RemixIcon name="mic-line" className="text-white text-lg animate-pulse" />
            </div>
            <div className="ml-3 bg-[#36219c]/40 backdrop-blur-md p-4 rounded-2xl rounded-tl-none border border-[#FF66C4]/30 flex items-center shadow-lg">
              <p className="text-sm mr-4 text-white font-medium font-accent">Escuchando<span className="dot-typing"></span></p>
              <div className="flex items-end space-x-1 h-6">
                {[1, 2, 3, 4, 5].map((_, i) => (
                  <div 
                    key={i} 
                    className="audio-wave w-1.5 rounded-full"
                    style={{ 
                      height: `${Math.max(4, Math.floor(Math.random() * 24))}px`,
                      animationDelay: `${i * 0.1}s`,
                      background: `linear-gradient(to top, #FF66C4, #FF9F5B)`
                    }}
                  ></div>
                ))}
              </div>
